...
public interface EmployeeRepository extends JpaRepository<Employee, Long> {
	...
	@Query(value = "select * from mockito.employees e where e.first_name = ?1 and e.last_name = ?2", nativeQuery = true)
	Employee findByNativeSQLFirstNameLastName(@Param("fName")String firstName, @Param("lName")String lastName);
	
	@Query(value = "select * from mockito.employees e where e.first_name = :fName and e.last_name = :lName", nativeQuery = true)
